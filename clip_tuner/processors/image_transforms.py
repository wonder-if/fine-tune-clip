...
def build_image_transforms(transform_cfg, image_processor=None) -> Callable:
    print("transform_cfg: {transform_cfg}")
    ...
    if image_processor is not None:
        transform_cfg.common.image_size = image_processor.image_size
    ...
    if transform_cfg.validation:
        # 处理 interpolation 字段
        for t in transform_cfg.validation.transforms:
            fix_interp(t)

        pipline = instantiate(transform_cfg.validation)
        logger.info(f"Validation transforms: {pipline}")
